...
function DraggableCard({
	...
	const handleShow = async (e: React.MouseEvent) => {
		...
		try {
			...
			const latest =
				Array.isArray(imagesFromApi) && imagesFromApi.length > 0
					? imagesFromApi[0]
					: imagesFromApi;
			...
			if (latest) {
				...
				onDescriptionsUpdate(latest.id, latest.short_desc || "", latest.long_desc || "");
			} else {
				setTempTitle(getTitle(image.short_desc, index));
				setTempLongDesc(getDesc(image.long_desc));
			}
						...
						try {
							...
							if (!updatedImage.long_desc_generating && updatedImage.long_desc) {
								...
								const updatedImageMetadata =
									await formatImageMetadata(updatedImage);
								logAction(
									{ actionType: "click", elementId: "description-generated" },
									{
										imageId: image.id,
										image_metadata: imageMetadataRef.current,
										updated_image_metadata: updatedImageMetadata,
	...
	const handleTitleSave = async () => {
		...
		logAction(
			{ actionType: "click", elementId: "visual-inline-title-save" },
			{ imageId: image.id },
		);
	...
	const handleDescSave = async () => {
		...
		logAction(
			{ actionType: "click", elementId: "visual-inline-desc-save" },
			{ imageId: image.id },
		);
					...
					<div id="card-footer" className="p-2">
						...
						{image.source === "instructor" && (
							<div className="flex items-center gap-1.5 mt-3 flex-wrap">
								<span className="inline-block text-somewhat-tiny font-medium text-white bg-red-400 rounded-full px-2 py-0.5">
									Instructor
								</span>
								{image.last_saved && (
									<span
										...
										style={{ fontSize: "0.5rem" }}
									>
										{new Date(image.last_saved).toLocaleString([], {
											month: "short",
											day: "numeric",
											hour: "2-digit",
											minute: "2-digit",
