        ...
        () => ({
            ...
            item: () => {
                ...
                logAction(
                    ...
                    { scaffold_id: scaffold?.id, scaffold_type: 'shock-lead', old_position: { x: (item as any).oldX, y: (item as any).oldY }, new_position: { x: finalX, y: finalY } }
                );
            },
        ...
        const handleMouseUp = () => {
            if (isDragging) {
                ...
                if (dragStartPosition.current && onPositionUpdate) {
                    ...
                    logAction(
                        { actionType: 'drag', elementId: 'scaffold-drag' },
                        { scaffold_id: scaffold?.id, scaffold_type: 'shock-lead', old_position: dragStartPosition.current, new_position: { x: position.x, y: position.y } }
                    );
                }
        ...
        >
            <div className="flex justify-between items-center p-2 bg-bama-crimson text-white rounded-t-sm">
                ...
                <button
                    onClick={(e) => {
                        e.stopPropagation();
                        logAction(e, { scaffold_id: scaffold?.id, scaffold_type: 'shock-lead' });
                        onClose();
                    }}
