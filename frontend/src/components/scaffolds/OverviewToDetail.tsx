...
const OverviewToDetail = ({
    ...
    const labels = SCAFFOLD_GROUP_LABELS[scaffoldNumber] || { 1: 'Overview', 2: 'Details' };

    const leftGroupId = 'overview-detail-overview';
    const rightGroupId = 'overview-detail-details';
        ...
        () => ({
            ...
            item: () => {
                ...
                logAction(
                    ...
                    { scaffold_id: scaffold?.id, scaffold_type: 'overview-to-detail', old_position: { x: (item as any).oldX, y: (item as any).oldY }, new_position: { x: finalX, y: finalY } }
                );
            },
        ...
        const handleMouseUp = () => {
            if (isDragging) {
                ...
                if (dragStartPosition.current && onPositionUpdate) {
                    ...
                    logAction(
                        { actionType: 'drag', elementId: 'scaffold-drag' },
                        { scaffold_id: scaffold?.id, scaffold_type: 'overview-to-detail', old_position: dragStartPosition.current, new_position: { x: position.x, y: position.y } }
                    );
                }
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center p-2 bg-bama-crimson text-white rounded-t-sm">
                <h3 className="text-sm font-bold">Overview to Detail</h3>
                <button
                    onClick={(e) => {
                        e.stopPropagation();
                        logAction(e, { scaffold_id: scaffold?.id, scaffold_type: 'overview-to-detail' });
                        onClose();
                    }}
            ...
            <div className="flex flex-row gap-4 p-2">
                <TwoColumnGroup
                    id={leftGroupId}
                    title={labels[LEFT_GROUP_NUMBER] || 'Overview'}
                    cards={leftCards}
                ...
                <TwoColumnGroup
                    id={rightGroupId}
                    title={labels[RIGHT_GROUP_NUMBER] || 'Details'}
                    cards={rightCards}
                    onCardAdd={handleCardAdd}
                    onCardRemove={handleCardRemove}
                    scaffoldId={scaffold?.id}
...
const TwoColumnGroup = ({
    ...
    const [{ isOver, canDrop }, drop] = useDrop(
        () => ({
            accept: ['image', 'group'],
            drop: (item: DragItem) => {
                if (item.id === scaffoldDragId) return { droppedInGroup: false };
            ...
            canDrop: (item: DragItem) => {
                if (item.id === scaffoldDragId) return false;
                if (item.type !== 'group') return item.groupId !== id && cards.length < 3;
                return item.type === 'group' && item.scaffoldId !== scaffoldId;
            },
            collect: (monitor) => ({ isOver: monitor.isOver(), canDrop: monitor.canDrop() })
        }),
        [id, onCardAdd, cards.length, scaffoldId, scaffoldGroupNumber, onGroupAdd, scaffoldDragId]
    );
...
export default OverviewToDetail;
