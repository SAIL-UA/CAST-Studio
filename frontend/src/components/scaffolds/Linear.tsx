...
const LinearSlot = ({
        ...
        drop: (item) => {
            ...
            if (item.type === 'group' && item.id) {
                if (onGroupAdd) {
                    ...
                    logAction({ actionType: 'drop', elementId: 'scaffold-group-add' }, { scaffoldType: 'linear' });
                }
            } else if (item.id) {
                onCardAdd(item.id, slotNumber);
                logAction({ actionType: 'click', elementId: 'scaffold-card-add' }, { scaffoldType: 'linear', groupNumber: slotNumber });
            }
        },
        collect: (monitor) => ({
    ...
    return (
        <div ref={setSortableRef} style={{ ...sortableStyle, display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
            {/* Number label above */}
            <span style={{ fontSize: 10, fontWeight: 600, color: '#005c84', marginBottom: 4, marginLeft: 2 }}>{displayNumber}</span>
                ...
                >
                    ...
                    {!readOnly && (
                        <div style={{ position: 'absolute', top: 5, right: 5, display: 'flex', gap: 3 }}>
                            {/* Duplicate */}
                            <button
                                onClick={(e) => { e.stopPropagation(); window.dispatchEvent(new CustomEvent('createScaffold', { detail: { pattern: 'linear' } })); }}
