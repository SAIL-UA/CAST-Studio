...
const TimeBased = ({
    ...
    // When scaffold or images change, set display count from data so reload restores 2–4 columns
    useEffect(() => {
        setDisplaySlotCount((prev) => Math.max(prev, visibleSlotCountFromData));
    }, [visibleSlotCountFromData]);

    // Local state: which card ids are in which period (1..4)
    const [periodCardIds, setPeriodCardIds] = useState<Record<number, Set<string>>>({
        1: new Set(),
        2: new Set(),
        3: new Set(),
        4: new Set()
    ...
    useEffect(() => {
        if (!scaffold) return;
        const next: Record<number, Set<string>> = { 1: new Set(), 2: new Set(), 3: new Set(), 4: new Set() };
        images.forEach((img) => {
            if (img.scaffoldId === scaffold.id && img.scaffold_group_number != null && img.scaffold_group_number >= 1 && img.scaffold_group_number <= 4) {
                next[img.scaffold_group_number].add(img.id);
            }
        });
        setPeriodCardIds(next);
    }, [scaffold, images]);

    const getPeriodFromGroupId = (groupId: string): number | null => {
        const m = groupId.match(/^time-based-period-(\d+)$/);
        return m ? parseInt(m[1], 10) : null;
    };

    const handleCardAdd = async (cardId: string, groupId: string) => {
        if (!scaffold) return;
        const period = getPeriodFromGroupId(groupId);
        if (period == null || period < 1 || period > MAX_SLOTS) return;

        await updateImageData(cardId, {
            scaffoldId: scaffold.id,
            scaffold_group_number: period
    ...
    const handleRemovePeriod = async () => {
        ...
        const removedPeriod = displaySlotCount;

        const imagesInPeriod = images.filter(
        ...
        for (const group of groupsInPeriod) {
            if (onGroupRemove) onGroupRemove(group.id);
        }

        setPeriodCardIds((prev) => {
            const next = { ...prev };
            next[removedPeriod] = new Set();
            return next;
        });
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center p-2 bg-bama-crimson text-white rounded-t-lg">
                <h3 className="text-sm font-bold flex items-center gap-1.5"><svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="2" width="10" height="5" rx="1"/><rect x="3" y="9" width="10" height="5" rx="1"/></svg>Narrative Structure: Timeline</h3>
            ...
            <div className="flex flex-row gap-4 p-2">
                {Array.from({ length: displaySlotCount }, (_, i) => i + 1).map((periodNum) => (
                    <TimeBasedPeriod
                        key={periodNum}
                        id={`time-based-period-${periodNum}`}
                        title={groupLabels[periodNum] || `Period ${periodNum}`}
                        cards={images.filter((img) => periodCardIds[periodNum]?.has(img.id))}
                        onCardAdd={handleCardAdd}
                        onCardRemove={handleCardRemove}
                        scaffoldId={scaffold?.id}
                        scaffoldGroupNumber={periodNum}
                        groups={(scaffold?.groups || []).filter((g) => g.scaffold_group_number === periodNum)}
...
// Period column (mirrors CauseEffectGroup)
type TimeBasedPeriodProps = {
    id: string;
    title: string;
    cards: ImageData[];
    ...
    const [{ isOver, canDrop }, drop] = useDrop(
        () => ({
            ...
            canDrop: (item: DragItem) => {
                if (item.id === 'time-based-scaffold') return false;
