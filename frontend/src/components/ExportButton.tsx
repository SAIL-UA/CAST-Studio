	...
	const handleExport = async (
		...
		try {
			...
			const blob = new Blob([resp.data], {
				type: String(resp.headers["content-type"] || fallbackMime),
			});
			...
			a.download =
				match?.[1] ||
				`data-story-${new Date().toISOString().replace(/[:.-]/g, "")}.${format}`;
			...
			showAlert({
				level: "error",
				message: "An error occurred while exporting the story. Please try again.",
			});
		}
	};
		...
		<DropdownMenu.Root open={open} onOpenChange={setOpen}>
			...
			<DropdownMenu.Portal>
				<DropdownMenu.Content
					className="mt-1 ml-1 shadow-lg z-400 bg-white rounded-lg py-1 min-w-35 overflow-visible"
					sideOffset={4}
					align="start"
				...
				>
					<DropdownMenu.Item
						...
						onSelect={(e) => {
							e.preventDefault();
							handleExport(e as unknown as React.SyntheticEvent, "pdf");
						}}
					>
						PDF
					</DropdownMenu.Item>
					...
					<DropdownMenu.Item
						...
						onSelect={(e) => {
							e.preventDefault();
							handleExport(e as unknown as React.SyntheticEvent, "docx");
						}}
					>
						DOCX
					</DropdownMenu.Item>
