...
import {
	getStoryUserEdited,
	setStoryUserEdited,
	STORY_EDIT_STATE_EVENT,
} from "@/utils/storyEditState";
import {
	STORY_STREAM_START,
	STORY_STREAM_CHUNK,
	STORY_STREAM_END,
...
const CraftStoryButton = ({
	...
	useEffect(() => {
		if (storyLoading && stageName) {
			window.dispatchEvent(
				new CustomEvent(STORY_GENERATION_STAGE, { detail: { stageName } }),
			);
		}
		...
		ws.onmessage = (event) => {
			try {
				...
				if (data.type === "start") {
					...
					window.dispatchEvent(
						new CustomEvent(STORY_STREAM_CHUNK, {
					...
					if (onStoryGenerated) {
						onStoryGenerated().catch((refreshError) => {
							console.error(
								"Error refreshing image data after story generation:",
								refreshError,
							);
						});
					}
					...
					window.dispatchEvent(
						new CustomEvent(STORY_REASONING_READY, {
							detail: {
								sequence_summary: data.sequence_summary,
								rq_reasoning: data.rq_reasoning,
							},
		...
		const handleGenerateEvent = (e: Event) => {
			...
			if (scaffoldId && !storyLoading) {
				...
				const syntheticEvent = {
					preventDefault: () => {},
					currentTarget: document.getElementById("craft-story-button"),
				} as any;
	...
	useEffect(() => {
		if (error && storyLoading) {
			showAlert({
				level: "error",
				message: `Story generation failed during: ${stageName}\n\n${error}`,
			});
			setStoryLoading(false);
			setTaskId(null);
			closeStreamSocket();
		}
	}, [error, storyLoading, stageName, showAlert, setStoryLoading]);
	...
	const runValidationAndGenerate = async (ctx: any) => {
		...
		const storyboardItems = images.filter(
			(img) => img.in_storyboard && img.source !== "instructor",
		);
		...
		const hasValidDescription = (img: ImageData) => {
			return (
				img.long_desc &&
				img.long_desc.trim() !== "" &&
				img.long_desc !== DESCRIPTION_PLACEHOLDER
			);
		};
		...
		if (scaffoldId) {
			...
			if (scaffoldItems.length === 0) {
				showAlert({
					level: "warning",
					message: "Add visuals to the scaffold to continue.",
				});
				return;
			}

			const scaffoldImages = scaffoldItems.filter(
				(img) => img.filepath && img.filepath !== "",
			);
	...
	const startGeneration = async (ctx: any) => {
		...
		try {
			...
			try {
				...
				const readyImages = backendImages.filter(
					(img: any) => img.in_storyboard && img.long_desc && img.long_desc.trim(),
				);
				console.log(
					`Images ready for story generation: ${readyImages.length}/${backendImages.length}`,
				);
			} catch (error) {
			...
			if (scaffoldId && scaffolds.length > 0) {
				...
				if (targetScaffold) {
					storyStructureId =
						SCAFFOLD_NUMBER_TO_PATTERN[targetScaffold.number] ||
						selectedPattern ||
						undefined;
				}
			...
			if (taskResponse.status === "success" && taskResponse.task_id) {
				...
				const pollForCompletion = async () => {
					...
					showAlert({
						level: "warning",
						message:
							"Story generation is taking longer than expected. Please check back in a few minutes.",
					});
					setStoryLoading(false);
					setTaskId(null);
					closeStreamSocket();
				...
				showAlert({
					level: "error",
					message: `Error starting story generation: ${taskResponse.message}`,
				});
				setStoryLoading(false);
				closeStreamSocket();
			}
	...
	const scaffoldOptions = scaffolds.map((s, idx) => ({
		id: s.id,
		label: `${
			SCAFFOLD_NUMBER_TO_PATTERN[s.number]
				?.split("_")
						...
						>
							{storyLoading && (
								<div
									...
									style={{ width: `${progress}%`, backgroundColor: "#348b94" }}
								/>
							)}
					...
					<DropdownMenu.Portal>
						...
						>
							<DropdownMenu.Item
								...
								onSelect={(e) => {
									targetScaffoldIdRef.current = null;
									handleCraft(e as any);
								}}
							>
								All workspace
							</DropdownMenu.Item>
							...
							{scaffoldOptions.map((opt, idx) => (
								<DropdownMenu.Item
									...
									className="block w-full text-left text-sm text-grey-darkest px-3 py-1.5 hover:bg-grey-lighter cursor-pointer outline-none"
									onSelect={(e) => {
										targetScaffoldIdRef.current = opt.id;
										handleCraft(e as any);
									}}
								>
									{idx + 1}. {opt.label}
