...
interface ResearchQuestionsContextValue {
	...
	questions: RqSummary[];
	/** Refetch links. Pass host id / submission id when viewing someone else's canvas. */
	refreshRqLinks: (targetUser?: string, submissionId?: string) => Promise<void>;
...
export const ResearchQuestionsProvider = ({ children }: { children: React.ReactNode }) => {
	...
	const refreshRqLinks = useCallback(async (targetUser?: string, submissionId?: string) => {
		targetUserRef.current = targetUser;
		submissionIdRef.current = submissionId;
		try {
			const data = await getResearchQuestions(targetUser, submissionId);
	...
	const toggleCardLink = useCallback(
		async (rqId: string, cardId: string, isGroup: boolean) => {
			...
			try {
				...
				await refreshRqLinks(targetUserRef.current, submissionIdRef.current);
				return false;
			}
		},
