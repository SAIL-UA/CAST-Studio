...
export const getImageDataAll = async (targetUser?: string, submissionId?: string) => {
...
export const getGroups = async (group_id?: string, targetUser?: string, submissionId?: string) => {
	...
	if (submissionId) {
		url +=
			(url.includes("?") ? "&" : "?") + `submission_id=${encodeURIComponent(submissionId)}`;
	}
...
export const getResearchQuestions = async (targetUser?: string, submissionId?: string) => {
	let url = "/research-questions/";
	const params: string[] = [];
	if (targetUser) params.push(`target_user=${encodeURIComponent(targetUser)}`);
	if (submissionId) params.push(`submission_id=${encodeURIComponent(submissionId)}`);
...
export const getNarrativeCache = async (targetUser?: string, submissionId?: string) => {
	const params: Record<string, string> = {};
	if (targetUser) params.target_user = targetUser;
	if (submissionId) params.submission_id = submissionId;
...
export const generateDescription = async (image_id: string) => {
	const response = await API.post(
		`/descriptions/generate/?image_id=${encodeURIComponent(image_id)}`,
	);
	return response.data;
};
...
export const logMousePositionBatch = async (
...
export const logScrollBatch = async (
	sessions: Array<{
		elementId: string;
		clientHeight: number;
		scrollHeight: number;
...
export const logUserAction = async (
	actionType: string,
	elementId: string,
	state_info?: Record<string, ImageData>,
) => {
...
export const getScaffolds = async (
	scaffold_id?: string,
	targetUser?: string,
	submissionId?: string,
) => {
	let url = scaffold_id
		? `/scaffolds/?scaffold_id=${encodeURIComponent(scaffold_id)}`
		: "/scaffolds/";
	if (targetUser) {
		url += (url.includes("?") ? "&" : "?") + `target_user=${encodeURIComponent(targetUser)}`;
	}
...
export type SubmissionAttemptMeta = {
	used: number;
	limit: number;
	remaining: number;
};
...
export const getSubmissionStatus = async () => {
	const response = await API.get("/submissions/status/");
	return response.data as SubmissionAttemptMeta;
};

export const createSubmission = async () => {
	const response = await API.post("/submissions/", {});
	return response.data as { submission: SubmissionRecord } & SubmissionAttemptMeta;
};

export const getSubmissions = async (targetUser: string) => {
	const response = await API.get("/submissions/", {
		params: { target_user: targetUser },
	});
	return response.data as { submissions: SubmissionRecord[] } & SubmissionAttemptMeta;
};

export const getSubmission = async (submissionId: string, targetUser: string) => {
