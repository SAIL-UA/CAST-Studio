...
class Workspace(models.Model):
    """
    Each user has exactly one live editor canvas.
    Immutable hand-ins live on Submission (former named snapshots).
    """

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, db_column="user_id", related_name="workspaces"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(fields=["user"], name="one_workspace_per_user"),
        ]


class Submission(models.Model):
    """
    Immutable student hand-in (evolved from named workspace snapshots).
    Canvas rows reference this via submission FK; story is frozen in narrative_snapshot.
    """
...
class MediaAsset(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, db_column="user_id", related_name="media_assets"
    )
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["user", "filepath"], name="uniq_user_media_filepath"
            ),
        ]
...
class UserAction(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, db_column="user_id", related_name="user_actions"
    )
    action = models.CharField(
        max_length=10, choices=ActionType.choices, default=ActionType.CLICK
    )
...
class ScrollLog(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, db_column="user_id", related_name="scroll_logs"
    )
...
class JupyterLog(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, db_column="user_id", related_name="jupyter_logs"
    )
...
class ScaffoldData(models.Model):
    ...
    valid_group_numbers = models.JSONField(
        default=list,
        help_text="List of valid group numbers for this scaffold (1=causes, 2=effects, etc.)",
    )
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=(
                    models.Q(workspace__isnull=False, submission__isnull=True)
                    | models.Q(workspace__isnull=True, submission__isnull=False)
                ),
                name="scaffold_workspace_xor_submission",
            ),
        ]
...
class GroupData(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, db_column="user_id", related_name="group_data"
    )
    workspace = models.ForeignKey(
        Workspace,
        on_delete=models.CASCADE,
    ...
    x = models.FloatField(default=0.0)
    y = models.FloatField(default=0.0)
    scaffold_id = models.ForeignKey(
        ScaffoldData,
        on_delete=models.SET_NULL,
        db_column="scaffold_id",
        null=True,
        blank=True,
        related_name="groups",
    )
    scaffold_group_number = models.IntegerField(
        null=True,
        blank=True,
        help_text="Group number within scaffold (1=causes, 2=effects, etc.)",
    )
    created_at = models.DateTimeField(auto_now_add=True)
    last_modified = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                ...
                name="group_workspace_xor_submission",
            ),
        ]
...
class ImageData(models.Model):
    """
    Image / note placement in a workspace or submission. File bytes live on MediaAsset.
    Notes have media=None.
    """

    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, db_column="user_id", related_name="image_data"
    )
    workspace = models.ForeignKey(
        Workspace,
        on_delete=models.CASCADE,
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                ...
                name="image_workspace_xor_submission",
            ),
            models.UniqueConstraint(
                fields=["workspace", "media"],
                condition=models.Q(media__isnull=False, workspace__isnull=False),
                name="uniq_workspace_media_placement",
            ),
            models.UniqueConstraint(
                fields=["submission", "media"],
                condition=models.Q(media__isnull=False, submission__isnull=False),
                name="uniq_submission_media_placement",
            ),
        ]
...
class SharedSession(models.Model):
    ...
    controlled_by = models.ForeignKey(
        ...
        blank=True,
        on_delete=models.SET_NULL,
        related_name="controlling_session",
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class SessionParticipant(models.Model):
    ...
    session = models.ForeignKey(
        SharedSession, on_delete=models.CASCADE, related_name="participants"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="session_participations"
    )
...
class ResearchQuestion(models.Model):
    """
    A research question belonging to a user's workspace or submission.
    ...
    images = models.ManyToManyField(
        ImageData, blank=True, related_name="research_questions"
    )
    groups = models.ManyToManyField(
        GroupData, blank=True, related_name="research_questions"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    last_modified = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                ...
                name="rq_workspace_xor_submission",
            ),
        ]
